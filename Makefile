...
# --- test_mdef_parser: .mdef controller definition parser tests (host-
# tested; see mdef_parser.h's header for the format spec and rationale).
MDEF_PARSER_SOURCES = mdef_parser.cpp test_mdef_parser.cpp
MDEF_PARSER_OBJECTS = $(MDEF_PARSER_SOURCES:.cpp=.o)
MDEF_PARSER_TARGET  = test_mdef_parser
$(MDEF_PARSER_TARGET): $(MDEF_PARSER_OBJECTS)
	$(CXX) $(CXXFLAGS) $(MDEF_PARSER_OBJECTS) -o $(MDEF_PARSER_TARGET) -lm

test: $(AIM_TARGET) $(FP_TARGET) $(SHOW_TARGET) $(EFFECTS_TARGET) $(SHOW_CONTROL_TARGET) $(PIXEL_MATRIX_TARGET) $(PROVISION_TARGET) $(LIVE_CONTROL_TARGET) $(WEB_PROTOCOL_TARGET) $(CONTROL_QUEUE_TARGET) $(PACING_TARGET) $(APPLY_TARGET) $(LUA_VM_TARGET) $(LUA_EFFECT_TARGET) $(GLOW_LUA_API_TARGET) $(GLOW_FENNEL_TARGET) $(SCRIPTS_STORAGE_TARGET) $(FX_ERROR_PIPELINE_TARGET) $(OSC_PARSER_TARGET) $(LITTLEFS_IMAGE_TARGET) $(BEAT_CLOCK_TARGET) $(BEAT_QUEUE_TARGET) $(MIDI_REALTIME_TARGET) $(DJLINK_PARSER_TARGET) $(DJLINK_MASTER_TARGET) $(SAFE_BLACKOUT_TARGET) $(MDEF_PARSER_TARGET)
	./$(AIM_TARGET)
	./$(FP_TARGET)
	./$(SHOW_TARGET)
	./$(EFFECTS_TARGET)
