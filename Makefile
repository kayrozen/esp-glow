...
# --- test_artnet_router: Wave 3 portable Art-Net packet building + per-
# universe routing/sequencing/ArtSync (host-tested; artnet_sink.cpp itself
# is device-only/HIL-only -- see tests/hil/test_l2_artnet.py) ---
ARTNET_ROUTER_SOURCES = artnet_router.cpp test_artnet_router.cpp
ARTNET_ROUTER_OBJECTS = $(ARTNET_ROUTER_SOURCES:.cpp=.o)
ARTNET_ROUTER_TARGET  = test_artnet_router
$(ARTNET_ROUTER_TARGET): $(ARTNET_ROUTER_OBJECTS)
	$(CXX) $(CXXFLAGS) $(ARTNET_ROUTER_OBJECTS) -o $(ARTNET_ROUTER_TARGET) -lm

# --- test_dmx_frame_buffer: B2 (SPEC-esp-glow-spiram-dmx) latest-wins DMX
# mailbox shared between the render task and the dmx_tx task (host-tested;
# dmx_sink.cpp/dmx_tx_task.cpp themselves are device-only) ---
DMX_FRAME_BUFFER_SOURCES = dmx_frame_buffer.cpp test_dmx_frame_buffer.cpp
DMX_FRAME_BUFFER_OBJECTS = $(DMX_FRAME_BUFFER_SOURCES:.cpp=.o)
DMX_FRAME_BUFFER_TARGET  = test_dmx_frame_buffer
$(DMX_FRAME_BUFFER_TARGET): $(DMX_FRAME_BUFFER_OBJECTS)
	$(CXX) $(CXXFLAGS) $(DMX_FRAME_BUFFER_OBJECTS) -o $(DMX_FRAME_BUFFER_TARGET) -lm

# --- test_artnet_discovery: Wave 3 Phase 3 ArtPoll/ArtPollReply parsing +
# discovery table (host-tested; see artnet_discovery.h's header comment for
# the byte-layout provenance) ---
ARTNET_DISCOVERY_SOURCES = artnet_discovery.cpp test_artnet_discovery.cpp
ARTNET_DISCOVERY_OBJECTS = $(ARTNET_DISCOVERY_SOURCES:.cpp=.o)
ARTNET_DISCOVERY_TARGET  = test_artnet_discovery
$(ARTNET_DISCOVERY_TARGET): $(ARTNET_DISCOVERY_OBJECTS)
	$(CXX) $(CXXFLAGS) $(ARTNET_DISCOVERY_OBJECTS) -o $(ARTNET_DISCOVERY_TARGET) -lm

test: $(AIM_TARGET) $(FP_TARGET) $(SHOW_TARGET) $(EFFECTS_TARGET) $(SHOW_CONTROL_TARGET) $(PIXEL_MATRIX_TARGET) $(PROVISION_TARGET) $(LIVE_CONTROL_TARGET) $(MDEF_TARGET) $(CONTROLLER_INIT_TARGET) $(USB_MIDI_PACKETIZER_TARGET) $(STATE_BROADCAST_TARGET) $(LED_FEEDBACK_TARGET) $(WEB_PROTOCOL_TARGET) $(CONTROL_QUEUE_TARGET) $(PACING_TARGET) $(APPLY_TARGET) $(WLED_TARGET) $(LUA_VM_TARGET) $(LUA_EFFECT_TARGET) $(GLOW_LUA_API_TARGET) $(GLOW_FENNEL_TARGET) $(SCRIPTS_STORAGE_TARGET) $(FX_ERROR_PIPELINE_TARGET) $(OSC_PARSER_TARGET) $(LITTLEFS_IMAGE_TARGET) $(BEAT_CLOCK_TARGET) $(BEAT_QUEUE_TARGET) $(MIDI_REALTIME_TARGET) $(DJLINK_PARSER_TARGET) $(DJLINK_MASTER_TARGET) $(SAFE_BLACKOUT_TARGET) $(DEVICE_CONFIG_TARGET) $(ARTNET_ROUTER_TARGET) $(ARTNET_DISCOVERY_TARGET) $(DMX_FRAME_BUFFER_TARGET)
	./$(AIM_TARGET)
	./$(FP_TARGET)
	./$(SHOW_TARGET)
	./$(EFFECTS_TARGET)
	...
	./$(ARTNET_ROUTER_TARGET)
	./$(ARTNET_DISCOVERY_TARGET)
	./$(DMX_FRAME_BUFFER_TARGET)
...
clean:
	rm -f $(AIM_OBJECTS) $(AIM_TARGET) $(FP_OBJECTS) $(FP_TARGET) $(SHOW_OBJECTS) $(SHOW_TARGET) \
	      ...
	      $(ARTNET_ROUTER_OBJECTS) $(ARTNET_ROUTER_TARGET) \
	      $(ARTNET_DISCOVERY_OBJECTS) $(ARTNET_DISCOVERY_TARGET) \
	      $(DMX_FRAME_BUFFER_OBJECTS) $(DMX_FRAME_BUFFER_TARGET) \
