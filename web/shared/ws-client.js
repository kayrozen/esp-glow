//
// ws-client.js — WebSocket client + command layer, shared copy for the
// provisioner's device-sync panel (§5: push/pull .fnl shows to a live
// rig). This is the same protocol/implementation as web/console/ws-client.js
// (the device console's live REPL/editor talks to the identical device-
// side handler) -- kept as an intentional copy rather than an import
// across web/console/ (which isn't published alongside the GitHub Pages
// provisioner deploy; see .github/workflows/provisioner.yml's "Copy shared
// assets" step, which only copies web/shared/ and web/vendor/) or restructured
// into firmware/main/data/console/ (embedded on-device, built by
// firmware/components/glow_core/CMakeLists.txt from that exact path).
//
// If the wire protocol ever changes, update BOTH copies -- same
// duplication trade-off this codebase already makes for
// web/console/shared/devcfg.js vs. firmware/main/data/console/shared/devcfg.js.
//
// Protocol (mirrored by web_protocol.cpp):
//
//   Device -> UI:
...
export class WsClient {
  ...
  constructor(url, opts = {}) {
    ...
    this.cb = {
      ...
      state:      null,  // (activeIds, masterLevel|null) -> void
