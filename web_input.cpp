...
#include "artnet_nodes_web.h"  // Wave 3 Phase 3: /artnet_nodes registers onto this same httpd server
...
void web_server_task(void* /*ctx*/) {
  ...
  // (CFG1 §6 -- see device_config_web.h), +1 for GET /artnet_nodes (Wave 3
  // Phase 3 -- see artnet_nodes_web.h) -- all reuse this same server rather
  // than starting a second one.
  config.max_uri_handlers = kNumStaticFiles + 5;
  ...
  artnet_nodes_web_register_handlers(g_server);

  ESP_LOGI(TAG, "web console + /ws + /ota + /devcfg + /artnet_nodes ready");
}
